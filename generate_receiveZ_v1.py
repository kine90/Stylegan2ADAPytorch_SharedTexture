...
def main(
    ...
    gp = []
    for i in range(pnum):
        print("Starting generator process num {}".format(i))
        gpi = mp.Process(target=gen_proc, args=(w[i], p[i], now_gen_par, outtex.ipc_handle, device))
        gpi.daemon = True
        gpi.start()
        gp.append(gpi)
        time.sleep(5)
    ...
    while True:
        ...
        for i in range(pnum):
            ...
            #LUNCH GEN PROC
            #print("Passing generation job to process {}".format(i))
            w[i].put(w_samples)
            

            try:
                perf[i] = p[i].get_nowait()
                #print("Process {} performance: {}s".format(i, perf[i]))
            except: pass
            if pnum > 1:
                for f in range(pnum):
                    totperf = perf[i] + totperf
                avgperf = totperf/pnum
            else: avgperf = perf[i]
            #print("Sleeping for {}s".format((avgperf/pnum)*0.999))
            time.sleep((avgperf/pnum)*0.99)
            firstrun = False
        elaps = (time.perf_counter() - t0)
        print("GENERATED {} FRAMES. FPS {} - HANDLE = {}".format(pnum, (1/(elaps/pnum)), outtex.ipc_handle))
